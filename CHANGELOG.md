...
- **The Lynx daisyUI template renders styled** (#124). Its `src/main.tsx` mounted `<App />` without `<ThemeProvider>`, so the daisyUI tokens the preset's classes resolve through (`--color-*`, `--text-*`) were undefined and the app rendered unstyled on device. It also imported its Tailwind utilities before the component styles, so component defaults beat utility classes (`Card.Actions class="justify-center"` stayed right-aligned). `App.tsx` moves to the current `color=` / `variant=` split on `Button` and `Badge`; `tsc` rejected the old props. The template also needs the `@sigx/lynx-daisyui` release carrying signalxjs/lynx#1137 before it bundles, since the root barrel imported optional peers until then.
- **Scaffolded Lynx apps build again** (#122). The three Lynx overlays hand-pinned `@sigx/lynx-*` `^0.12.1`, `@sigx/reactivity`/`@sigx/runtime-core` `^0.7.0` and `@sigx/cli` `^0.5.0`, and left the `@lynx-js/*` toolchain unbounded (`>=0.1.0`). `npm create @sigx -- --kind lynx` therefore produced an app whose first `sigx build` failed: rspeedy resolved to 0.18 (Rsbuild 2), which lynx-plugin 0.12 cannot drive (`` `env` and `jsc.target` cannot be used together ``). The overlays now write `{{dep:<name>}}` placeholders that `sigx create` resolves through `dep()`, so their versions come from the generated `versions.ts` like every other template's. The Lynx release line (`@sigx/lynx` `^0.32.0` — lynx v0.32.0 carries signalxjs/lynx#1137, which the daisyUI template needs to bundle — shared by every lockstep `@sigx/lynx-*` package) is in the catalog. The `@lynx-js/*` pins in `template-versions.json` follow the toolchain signalxjs/lynx tests against: rspeedy `^0.16.3`, and `template-webpack-plugin` exactly `0.15.0` because lynx-plugin peers it exactly. A test fails if a Lynx overlay hand-pins a `@sigx/*` or `@lynx-js/*` range again.
...
- **The server-function endpoint in the Bun, Cloudflare, Deno, Netlify and Vercel entries passes the registry as `functions: serverFns`** — core 1.0 (`@sigx/server` rfc-server-v5 §1.6) made `functions` the primary endpoint option and turned registry entries into `{ version, load }`, so the old `resolve: (symbol) => serverFns[symbol]?.() ?? null` escape hatch no longer typechecks (`TS2349: This expression is not callable`). The Node `server.mjs` already used `functions`.
  ...
  A project scaffolded with `--features server-fn,testing` failed its test suite on core 1.0.0 — `@sigx/vite`'s client-stub HMR tail read `import.meta.hot.data`, which vitest's no-op hot context does not have (signalxjs/core#726). Nothing in the template was wrong; core 1.0.1 fixed it, and a fresh `sigx create` install resolves it through the `^1.0.0` pin.
...
- **The SSR template moves to the current core shape**: `vite build --app` (one build for client and server), `src/entry-server.tsx` exporting the per-request `createApp(url)` factory, `server.mjs` on `createDevRequestHandler` / `createRequestHandler` with the build's `dist/server/sigx-app.js`, and `--conditions production` for the production run. The pre-0.15 `render()` + `vite.ssrLoadModule` shape is gone.

- **Every generated project now ships a `.gitignore` and a README** describing its layout, scripts and how to deploy, with commands spelled for the package manager that will run them (`pnpm dev` / `npm run dev` / `bun run dev` / …). "Next steps" after scaffolding come from the same source.

### Added

- **A new `sigx create` wizard, and install + git after scaffolding** (#108, part 4 of #100). The interactive flow is now: project name (validated — no path separators, directory must not exist) → **Quick start** (SPA + Tailwind + router + tests) or **Customize** (kind → rendering + deploy target for SSR, with an Edge-runtime heads-up → styling → extras) → package manager (defaults to the one that ran `create`, from `npm_config_user_agent`, else the nearest lockfile) → install dependencies? → initialize git? (default off when already inside a repository) → a summary and a final confirmation (No goes back once) → spinner phases for scaffolding, `<pm> install` (output captured; the tail is shown on failure, which never fails the run) and `git init` + first commit (a scaffold-only identity via `-c` when none is configured). "Next steps" are spelled for the chosen package manager and drop the install line when it already ran. Esc/Ctrl+C at any prompt still exits 130.

  Headless gains the full flag surface — `--kind` (with `--type` kept as a deprecated alias; the two disagreeing is an error), `--render`, `--target`, `--styling`, `--features`, `--pm`, `--install/--no-install`, `--git/--no-git`, `--preset quick`, `-y`, and `--list` (prints every kind, render mode, target, styling and extra this build can generate). Headless never installs or touches git unless asked. Flags given alongside the wizard skip its preset question and become the prompts' initial values. The command is now a thin shell over `src/create/headless.ts` (flags → spec), `src/create/wizard.ts` and `src/create/postinstall/{install,git,pm}.ts`; `@sigx/cli/create`'s `CreateOptions` type grew accordingly.

- **Extras — `--features router,i18n,testing,server-fn`** (#104, part 3 of #100), also an "Extras" multiselect in the interactive flow, filtered to what the chosen kind/render/target supports. Each is a layer: **router** (`@sigx/router`: `src/router.ts`, Home/About pages, `<RouterView />`, `createWebHistory` in the browser and a per-request `createMemoryHistory` on the server), **i18n** (`@sigx/i18n`: an `app` namespace in English and Swedish under `src/locales/`, lazy catalogs, `?lang=`/cookie/browser detection, SSR preload + state transfer, a language switch in the header), **testing** (Vitest on happy-dom + oxlint with `test`/`lint` scripts, `vitest.config.ts`, `.oxlintrc.json` and a sample test that mounts the App — or the Counter island on a static site), **server-fn** (`@sigx/server`: `src/api/hello.server.ts` declared with `serverFn`, read from `src/components/ServerGreeting.tsx` with `useData`; the endpoint composes into the platform entry through the existing `@sigx:if server-fns` blocks, and `sigxServer()` joins `vite.config.ts` without displacing resume's `renderBoundaries` call). The kind overlays now carry `@sigx:if router|i18n|server-fn` hooks so every feature combination shares one `App.tsx`, `main.tsx` and pair of SSR entries; the counter moved to `src/components/Counter.tsx`. Router and i18n require a hydrated client (SPA, or SSR in `hydrate` mode); server functions require SSR; testing covers SPA, SSR and SSG. **Actors** is deferred to #106 — its host/endpoint/Durable Object wiring deserves its own PR.

- **Deploy targets for SSR projects — `--target node|cloudflare|bun|deno|vercel|vercel-edge|netlify`** (#103, part 2 of #100). Each target is a layer: the platform entry (`src/entry.cloudflare.ts`, `server.bun.ts`, `src/entry.deno.ts`, `src/entry.vercel.ts`, `src/entry.netlify.ts`), its config (`wrangler.jsonc`, `deno.json`, `netlify.toml`), the adapter in `vite.config.ts` (`cloudflare()`, `vercel({ runtime: 'edge' })`, `netlify()`, the inline Deno adapter), `deploy`/`start` scripts, the README's deploy section, and a deploy hint under "Next steps". Every target keeps `server.mjs` so `dev` is the same Vite dev server everywhere; Express becomes a dev-only dependency off Node. The interactive flow asks for the target after picking SSR.

- **Render modes for SSR projects — `--render hydrate|islands|resume`**. `islands` (`@sigx/ssr-islands`): a server-only page whose components hydrate per `client:*` directive, with an app-less `hydrateIslands()` client. `resume` (`@sigx/resume` + `@sigx/server`): zero JavaScript on load, handlers extracted into chunks, `refreshComponents` + `createBoundaryRefresh` wired for single-flight boundary refresh in every entry, `sigxServer({ renderBoundaries: '/src/dev-refresh.ts' })` for dev. The server-function endpoint is composed into each platform entry only when the build carries a registry (`@sigx:if server-fns`), so a plain hydrated app ships none of it.

- **Opt-in end-to-end suite** (`SIGX_E2E=1 pnpm test -- e2e`, new `e2e-create` CI job on ubuntu + windows): scaffolds spa, ssr-node, ssr-cloudflare-resume, ssg and terminal with the built CLI, installs them from npm, and typechecks + builds each.

- **`--type terminal`** scaffolds a `@sigx/terminal` app: TSX + signals, `sigx-terminal-dev` for HMR, `tsx` to run it plainly.

- **Template versions are locked to the pnpm catalog** — closes #91. `packages/cli/src/create/versions.ts` is generated by `scripts/gen-versions.mjs` (`pnpm gen:versions`) from `pnpm-workspace.yaml` (the core line plus the new companion entries — `@sigx/router`, `@sigx/ssg`, `@sigx/daisyui`, `@sigx/terminal`, …), `@sigx/cli`'s own version, and `scripts/lib/template-versions.json` for third parties. `pnpm verify:catalog` now also runs `gen-versions --check`, so a bumped catalog with a stale file fails CI; `__tests__/create/versions.test.ts` asserts every `@sigx/*` range in every generated `package.json` matches. `pnpm sync:core X.Y` therefore moves the templates too.
