# SignalX cli — shared agent guide
...
SignalX `cli` (`signalxjs/cli`) — SignalX CLI — unified `sigx` binary, `@sigx/create` scaffolder, and web project templates. A pnpm monorepo (ESM, `"type": "module"`) with the packages under `packages/`. Tech stack: TypeScript (strict), Vitest, oxlint; published to npm under the `@sigx` scope.
...
Never commit straight to `main`.** Repo: `signalxjs/cli`, base branch `main`.
...
4. **Open a PR with Copilot as the reviewer.** Reference the issue so it auto-closes
   ...
   ```sh
   gh api --method POST repos/signalxjs/cli/pulls/<pr>/requested_reviewers \
...
5. **Wait for Copilot's review, then fix.** Do not merge before it has reviewed. Poll
   ...
   **Then resolve the threads.** The sigx-standard ruleset sets
   ...
   gh api graphql -f query='query { repository(owner:"signalxjs", name:"cli") {
...
- `packages/cli` → `@sigx/cli` — Unified CLI for SignalX — create, dev, build, and run projects.
- `packages/create` → `@sigx/create` — Create a new SignalX project — `npm create @sigx@latest my-app`.
...
- **Before merging a PR with user-facing changes, file an issue on the docs
  ...
  gh issue create --repo signalxjs/signalxjs.github.io \
    --title "cli: <what changed>" \
    --body "Source: signalxjs/cli#<pr>. <What needs documenting, and where on the site.> Not yet released."
  ```
...
4. Adapt the repo-specific sections of `AGENTS.md`: the intro (what the repo is),
   "Build, Test, Lint", and "Packages". Replace every `cli` with the repo name.
5. Keep the workflow, worktree, and conventions sections as-is — they are the
   shared standard.
6. Lock down `main`: `node scripts/apply-branch-protection.mjs signalxjs/cli`.
